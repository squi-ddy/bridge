...
export const cardSort = (a: {card: Card, valid: boolean}, b: {card: Card, valid: boolean}, balatro: boolean) => {
...
export function cardToCardURL(card: Card, balatro: boolean = false): string {
    if(balatro) return `${import.meta.env.BASE_URL}/cards/balatro/${cardSuitToStr[card.suit]}-${cardValueToStr[card.value]}.png`
    return `${import.meta.env.BASE_URL}/cards/${cardSuitToStr[card.suit]}-${cardValueToStr[card.value]}.svg`
